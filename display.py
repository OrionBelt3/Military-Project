...
class Display(QWidget):
    ...
    def __init__(self, root, graph_in, step = 50, max_time = -1, verticle=False, horizontal = False, late_time = None, base_graph = None, switch = True):
        ...
        self.scaler = 0
        self.radius = 30
        self.parametrScaler = 0.25 # в столько раз будет увеличиваться или уменьшаться радиус
    ...
    def zoomIn(self):
        ...
        self.graph.Points[0][0] = self.graph.Points[0][0] / (1 + (self.scaler - 1) * self.parametrScaler)
        self.graph.Points[0][1] = self.graph.Points[0][1] / (1 + (self.scaler - 1) * self.parametrScaler)
